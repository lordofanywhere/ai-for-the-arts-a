...
# ![Machine Learning model](workflow.png)
# **Source:** [Towards Data Science](https://towardsdatascience.com/workflow-of-a-machine-learning-project-ec1dba419b94)
# 
# <p><ul><li><b>Decision trees:</b></li><ul>
#     <li>The training dataset includes hiring data (if a hire is a good hire or not), linked to the favourite TV show of the candidates, and if these candidates have a computing degree or not. This data is used to create a decision tree.</li>
#     <li>The decision tree splits all candidates using two questions, resulting in three leaf nodes. This is the algorithm.</li>
#     <li>There is no evaluation provided in the example, but we know the algorithim is overfitting. We could use a confusion matrix to test this model.</li>
#     <li>There is no testing data provided for this example.</li></ul>
#     <li><b>Linear Regression:</b></li><ul>
#     <li>In the example, the training data is the size and weight of mice.</li>
#     <li>The model is a line that closely approximates the data, which is used to predict a continuous value.</li>
#     <li>The formula for linear regression is <i>y = 0.5x + 2</i>.</li>
#     <li>The evaluation data could be the squiggly line, which does not work as well as the straight line.</li></ul>
#     <li><b>Neural networks:</b></li><ul>
#     <li>The model seeks to detect if words represented as numbers on emails indicate that an email is spam or not.</li>
#     <li>The dataset is composed by emails.</li>
#     <li>The formula for this algorithm takes the number that represents the inputs, and multiplies them by a weight, adding (or subtracting) a bias.</li>
#     <li>There is no training or test data provided, but to train and test the model, the data should be split so that tarinign data and test data are selected consistently from the entire dataset.</li></ul>
#     </ul></p>
#     
#   <p>The model that I have found is easy to apply to any audience or scenario, as it depitcs the steps clearly and and in a very concrete way. There are terms that may need explanation, like 'testing dataset', 'training dataset' and 'production data', but once an example is given, it becomes quite easy to follow.
#     
#    It is important to relate to the audience, using examples that recall datasets that they may encounter. For example, for a museum curator, I would adapt the examples to the data from the museum's collections. That way, they museum curator will have a much more understandable example.</p>
...
# ***Markdown cell for discussions***
# 
# 1. I would gather the date the properties were built and the date the properties were last sold or independently valued, the property type and the size of the properties, as well as other details that may impact pricing (number of rooms, if it has a garden, etc). My knowledge is conditioned by the UK property market, and in different markets property values may be influenced by variables that I am not aware of, like proximity to facilities, schools, etc. For example, the [waitrose effect](https://www.theguardian.com/inequality/2017/oct/02/the-waitrose-effect-boom-times-for-homeowners-but-evictions-for-tenants) is a phenomenon only present in the UK.
# 
# 2.
# 
# 3.
...
# 
# 1. The size of each image is 28x28. The NIST original dataset uses the SD-3 dataset as training set and SD-1 as test set. SD-3 was collected amongst Census Bureau employees and it is easier to recognise, and SD-1 was collected amongst high-school students. MNIST was created by mixing both datasets (30,000 patterns from each for the training set, and 5,000 patterns from each for the test set). This was done to draw better conclusions. i think it was the right decision, as they are training the model with a wider range of handwritten patterns, making it less biased and potentially more accurate.
# <br>2.
# <br>3.
# <br>4.
...
# ***Markdown cell***
# 
# 1. This is a reasonable approach in order to keep the model representative of the different income bands, so that it is not skewed towards a specific income band, or no income band is under-represented or over-represented.
# 2. 
# 3.
# 4.
# 
...
# 
# 1. First, we would need to understand where the data comes, so that we check any potential bias that may impact the data. For example, in the case of the survey data, we would need to ensure that no categories are over-represented or under-represented. 
# 
# In the case of missing data, the approach would depend on the type of data is missing: if there is not a lot of missing data across various categories, I may consider level the data down (dropping missing values). However, if a few categories are missing many entries, I may consider using the median to fill out the missing entries (imputation). Unless we don't need a specific column with missing values, I would not consider dropping it altogether.
# 
# Data will need to be prepared by scaling features, so that the data use similar scales across all features.
# 
# In the case of images, we can resize them and turn them into grayscale so that they are processed faster by the model.
# 
# We need to understand the question we need to answer so that we can choose the right model and/or adapt the model accordingly. Otherwise we risk obvercomplicating our model (for example, using a regression model when a classification model would have been enough) or not answering the question appropriately.
# 
# Cross validation is useful to test how accurate is our model using new data that was not used to train it, and understand if there is overfitting or other provlems. Cross validation can also give us a better idea of the fit accuracy of our model used with our dataset, by testing our dataset using different models and comparing the accuracy of all the models used.
# 
# 2.<br>
# 3.
# 
...
# ### Task 5-2-2  
# 
# The model I tested had the following composition: spiral, 50% ratio of training to test, 0 noise and bacth size 10; with the properties X₁, X₂, X₁² and X₁X₂, 6 neurons in one hidden layer and 2 neurons in the second layer.
# 
# The output was 0.307 (for test loss) and 0.179 (for training loss) at epoch 009,096, which suggests overfitting. However, at some points, the test loss and training loss rate were very similar.
# 
# The fetures presents straight lines, but the first hidden layer introduces curves. When adding one more neuron in the second layer, the model tries to revert the movement introduced in the previous layer, probably trying to generate a spiral movement.
# 
